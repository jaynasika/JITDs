...
public class CrackerMode extends Mode
{
  ...
  public KeyValueIterator scan(Driver driver, long low, long high)
  {
    ...
    ////System.out.println("Pushdown "+ ((crackStartTime - pushdownStartTime) / 1000));
    if(low==0){
    	driver.root = crackOne(driver.root, high);
    }
    else{
    ...
    ////System.out.println("Crack Time "+ ((scanStartTime - crackStartTime) / 1000));
    
    KeyValueIterator kv = scan(driver.root, low, high);
    
    long endTime = System.nanoTime();
    log.info("Scan time: {} us", (endTime - scanStartTime) / 1000);
    //System.out.println("Scan Time "+ ((endTime - scanStartTime) / 1000));
    return kv;
  }
